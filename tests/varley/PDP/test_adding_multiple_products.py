...
class Test_Product_Page:   
    @pytest.mark.parametrize("product_handle, variant, color, size", [
        ("alana-slim-tapered-cuff-pant-29", "44372758266029", "Simply Taupe", "XXS"),
        # ("scott-scoop-neck-midi-dress", "44489831219373", "Birch", "XXS")
    ])
    ...
    def test_nav_to_page(self, product_handle, variant, color, size) -> None:
        url = f"https://www.varley.com/products/{product_handle}?variant={variant}&Color={color.replace(' ', '%20')}&size={size}"
        self.driver.get(url)
        print(f"➡️ Navigating to URL: {url}")
        time.sleep(2)

        # self.verify_location_modal()
        print(f"➡️ Navigating to URL: {url}")
        time.sleep(2)

        # self.verify_location_modal()
        # # self.verify_that_the_page_is_viewable()
        # # self.test_content()
        # # self.test_content_accordion()
        # # self.test_add_to_cart()
        # # self.test_complete_the_look()
        return url
   
    def test_verify_location_modal(self, driver) -> None:
        # self.setup_driver()
        # driver = self.driver
    ...
    def verify_that_the_page_is_viewable(self) -> None:
        print("Breadcrumbs is displayed")
        time.sleep(5)
        
        breadcrumbs = [item.text for item in self.driver.find_elements(By.CSS_SELECTOR, "nav ul li a")]
        print(f"Breadcrumbs: {' / '.join(breadcrumbs)}")

        homepage_link = self.driver.find_element(By.CSS_SELECTOR, 'a[href="/"]')
        home_page = homepage_link.get_attribute("href")
        if not home_page.startswith("http"):
            home_page = "https://www.varley.com" + home_page

        collection_breadcrumbs = self.driver.find_element(By.CSS_SELECTOR, 'a[href="/collections/knitwear-and-sweaters"]')
    ...
    def test_content(self) -> None:
        ...
        title = self.driver.find_element(By.CSS_SELECTOR, ".text-\[19px\]\/\[22px\]")  
        assert title.is_displayed(), "Title is not displayed"
        # assert title.text == test_results["h1"], "Title is incorrect"
        print("Title is correct")
        
        description = self.driver.find_element(By.CSS_SELECTOR, '.px-5 > .mb-5')
        ...
        product_price = self.driver.find_element(By.CSS_SELECTOR, '.px-5 > .text-black')
        assert product_price.is_displayed(), "Product Price is not displayed"
        print("Product Price is correct")
        
        color_text = self.driver.find_element(By.CSS_SELECTOR, '.items-center > .block')
        assert color_text.is_displayed(), "Color text label is not displayed"
        print("Color text label is displayed\n")
        
        color_section = self.driver.find_element(By.CSS_SELECTOR, '.gap-6 > .flex:nth-child(2)')
        ...
        size_chart_link = self.driver.find_element(By.CSS_SELECTOR, '.underline-offset-4:nth-child(2)')
        ...
        time.sleep(2)
        size_guide_modal = self.driver.find_element(By.CSS_SELECTOR, 'div[data-headlessui-state="open"]')
        ...
        container = self.driver.find_element(By.CSS_SELECTOR, 'div.flex.border.border-black')
        ...
        delivery_note_text = self.driver.find_element(By.CSS_SELECTOR, "div.my-10 p")
    ...
    def test_content_accordion(self) -> None:
        description_accordion = self.driver.find_element(By.CSS_SELECTOR, '[class^="px-5"] [class="overflow-hidden"]:nth-child(6)')
        self.driver.execute_script("arguments[0].scrollIntoView();", description_accordion)
        ...
        time.sleep(3)
        description_content = WebDriverWait(self.driver, 10).until(
        ...
        fit_measurement_button = self.driver.find_element(By.XPATH, "//button[contains(., 'Fit & Measurements')]")
        self.driver.execute_script("arguments[0].click();", fit_measurement_button)
        fit_measurement_content_panel = WebDriverWait(self.driver, 10).until(
        ...
        print("\nVerify Fabric & Care")
        fabric_care_button = self.driver.find_element(By.XPATH, "//button[contains(., 'Fabric & Care')]")
        self.driver.execute_script("arguments[0].click();", fabric_care_button)
        fabric_care_content_panel = WebDriverWait(self.driver, 10).until(
        ...
        delivery_return_button = self.driver.find_element(By.XPATH, "//button[contains(., 'Delivery & Returns')]")
        self.driver.execute_script("arguments[0].click();", delivery_return_button)
        delivery_return_content_panel = WebDriverWait(self.driver, 10).until(
    ...
    def test_add_to_cart(self) -> None:
        ...
        self.driver.execute_script("arguments[0].scrollIntoView();", add_to_bag_button)
        self.driver.execute_script("window.scrollBy(0, -100);")
        add_to_bag_button.click()
        time.sleep(3)
        print("Clicked the Add to bag button")
        
        sidecart = WebDriverWait(self.driver, 10).until(
    ...
    def test_complete_the_look(self)-> None:
        print("\nVerfiying the Complete the look section")
        complete_the_look = WebDriverWait(self.driver, 10).until(
            EC.element_to_be_clickable((By.CSS_SELECTOR, '[class^="px-5"] [class="overflow-hidden"]:nth-child(8) button'))
        )
        self.driver.execute_script("arguments[0].scrollIntoView();", complete_the_look)
        self.driver.execute_script("window.scrollBy(0, -100);")
        ...
        initial_buttons = WebDriverWait(self.driver, 10).until(
        ...
        for idx in range(len(initial_buttons)):
            quick_preview_buttons = WebDriverWait(self.driver, 10).until(
                EC.presence_of_all_elements_located((By.CSS_SELECTOR, quick_preview_selector))
            )
            ...
            quick_preview_modal = WebDriverWait(self.driver, 10).until(
