...
class Wholesale_Page:    
    def test_nav_to_page(self, env) -> None:
        driver = self.setup_driver()
        url = "https://www.varley.com/pages/wholesale"
    ...
    def verify_main_hero(self, driver):
        ...
        assert title_text == "Wholesale", f"Title verification failed. Found: '{title_text}'"
        print("Title verification passed: 'Wholesale'")
        
        expected_content = (
            "Email Us "
            "For any stockist queries please contact us at sales@varley.com "
        ...
        assert "sales@varley.com" in email_link, f"Email link mismatch! Found: {email_link}"
    ...
    def verify_successful_submission_of_form(self, driver):
        ...
        driver.find_element(By.CSS_SELECTOR, "input[name='businessName']").send_keys("Ecrubox")
        driver.find_element(By.CSS_SELECTOR, "input[name='city']").send_keys("Worcester")
        driver.find_element(By.CSS_SELECTOR, "input[name='state']").send_keys("Worcester")
        dropdown = Select(driver.find_element(By.CSS_SELECTOR, "select[name='country']"))
        ...
        # driver.execute_script("arguments[0].scrollIntoView(true);", success_message)
...
if __name__ == "__main__":
    ...
    wholesale_page.test_nav_to_page("prod")
