...
class Varley_Home_Page:    
    ...
    def verify_main_banner(self, driver):
        ...
        expected_main_hero = "Our New York Summer continues with our latest Club Collection."
    ...
    def verify_category_carousel(self, driver):
        ...
        expected_category_values = {
            "New Arrivals": "https://www.varley.com/collections/bestsellers",
        ...
        for category_name, expected_url in expected_category_values.items():
            ...
            if category_name in ["Activewear", "Sweatpants", "DoubleSoft®", "Tops"]:
    ...
    def verify_fourth_block(self,driver):
        ...
        expected_h2_texts = [
            "Dress season.",
            "Dressed for the court."
        ]
        expected_links = {
            "Shop Now": "https://www.varley.com/collections/dresses",
            "Shop Club Dresses": "https://www.varley.com/collections/active-dresses"
        }

        print("Verify titles")
        h2_elements = fourth_block_element.find_elements(By.CSS_SELECTOR, "h2.title")
        h2_texts = [h2.get_attribute("innerText").strip().replace("\n", " ") for h2 in h2_elements]
        assert all(text in h2_texts for text in expected_h2_texts), f"Unexpected h3 text: {h2_texts}"

        print("\nVerify secondary text and links")
        links = fourth_block_element.find_elements(By.CSS_SELECTOR,  "div.flex.lg\\:h-full a.text-body.underline")
        ...
        for link in links:
            ...
            if text in expected_links:
                found_links[text] = href
                assert href == expected_links[text], f"Incorrect URL for {text}: {href}"
                print(f"✅ {text} link is correct: {href}")
        missing_links = set(expected_links.keys()) - set(found_links.keys())
        assert not missing_links, f"Missing expected links: {missing_links}"

        print("\nAll expected links verified successfully!")
        ...
        print("\nVerify images")
        image_elements = fourth_block_element.find_elements(By.CSS_SELECTOR, 'picture[data-insider-id="blocks.split-4-tile-0-image"] img')
        for img in image_elements:
            img_src = img.get_attribute("src")
            response = requests.head(img_src)
            assert response.status_code == 200, f"Image {img_src} returned {response.status_code}"
