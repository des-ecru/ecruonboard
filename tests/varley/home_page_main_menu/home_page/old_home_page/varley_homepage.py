...
class Varley_Home_Page:    
    ...
    def verify_main_banner(self, driver):
        ...
        # MAIN HERO
        expected_h1 = "Meet our bestsellers."
        ...
        expected_main_hero = "Explore a collection of our most sought-after pieces and classic styles, made to last beyond the season."
        ...
        expected_button_link = "https://www.varley.com/collections/bestsellers"
    ...
    def verify_category_carousel(self, driver):
        ...
        expected_category_values = {
            ...
            "Outerwear": "https://www.varley.com/collections/jackets-and-coats",
            "Accessories": "https://www.varley.com/collections/accessories",
            "Activewear": "https://www.varley.com/collections/sherpa",
        ...
        for category_name, expected_url in expected_category_values.items():
            ...
            if category_name in ["Accessories", "Activewear", "Pants", "Sweatpants"]:
    ...
    def verify_third_block(self,driver):
        ...
        third_block_element = driver.find_element(By.CSS_SELECTOR , '[data-insider-id="blocks.carousel-2"]')
        driver.execute_script("arguments[0].scrollIntoView(true);", third_block_element)
        driver.execute_script("window.scrollBy(0, -100);")
        print(f"Navigated to 3rd block")

        # Expected text for assertion
        expected_links = [
            "https://www.varley.com/collections/new-arrivals",
            "https://www.varley.com/collections/hoodies-and-sweatshirts",
            "https://www.varley.com/collections/double-soft",
            "https://www.varley.com/collections/outerwear"
        ]

        image = driver.find_element(By.CSS_SELECTOR, 'div[data-insider-id="blocks.split-3-tile-0-video"] video')
        image_url = image.get_attribute('src')
        response = requests.get(image_url)
